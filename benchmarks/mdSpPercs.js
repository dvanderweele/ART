...
;(async ()=>{ 
  ...
  */ 
  const mdP95CountNode = mdCountIdx.percentile(95)
  const mdP95LengthNode = mdLengthIdx.percentile(95) 
  const spP95CountNode = spCountIdx.quantile(0.95)
  const spP95LengthNode = spLengthIdx.quantile(0.95)
  ...
  */ 
  const mdTop5PCount = [...mdCountIdx.ITER_FWD_FROM_QTL(0.95,[Infinity])].map(x=>x[1])
  const mdTop5PLength = [...mdLengthIdx.ITER_FWD_FROM_QTL(0.95,[Infinity])].map(x=>x[1]) 
  const spTop5PCount = [...spCountIdx.ITER_FWD_FROM_QTL(0.95,[Infinity])].map(x=>x[1])
  const spTop5PLength = [...spLengthIdx.ITER_FWD_FROM_QTL(0.95,[Infinity])].map(x=>x[1])
