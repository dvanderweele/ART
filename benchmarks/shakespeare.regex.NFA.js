...
const mobydick_file = path.join(__dirname,"shakespeare.txt")

;(async ()=>{
  const mobydick_raw = (await fs.readFile(
    mobydick_file,
    {encoding: "utf8"}
  )).replaceAll("’","'").replaceAll("‘","'").replaceAll("œ","oe").replaceAll("…","...")
  ...
  await fs.writeFile(path.join(__dirname,"shakespeare.regex.txt"),deduped.toRegex().serialize(),{encoding:"utf8"})
  ...
  let wc = 0
  const OOR = new Map()
  performance.mark("pre-RegExp-tests")
  for(let i=0;i<words.length;i++){
  ...
  performance.mark("post-RegExp-tests")
  ...
  console.log("RegExp tests duration (ms)", performance.measure("RegExp tests duration (ms)","pre-RegExp-tests","post-RegExp-tests").duration)
