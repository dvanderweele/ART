...
;(async ()=>{
  ...
  for(let prefix of prefixes){
    ...
    let rangeStart = probe
    while(rangeStart-1>0&&sorted[rangeStart-1].startsWith(ps)) rangeStart--
    while(rangeStart<sorted.length&&sorted[rangeStart].startsWith(ps)) rangeStart++ 
  }
  */
  performance.mark("prefix.iter.after")
  // 1000x contains substring queries
  const result = {
